...
class PaymentService {
  ...
  async verifyPayment(txRef) {
    ...
    if (response.status === 'success') {
      ...
      if (response.data.meta?.subscriptionId) {
        const subscription = await Subscription.findById(response.data.meta.subscriptionId);
        subscription.status = 'active';
        subscription.flutterwaveSubscriptionId = response.data.payment_plan?.subscription_id || txRef; // Fallback to txRef if no subscription ID
        ...
        const claimToken = await generateClaimToken(transaction._id, usdcEquivalent);
  ...
  async cancelSubscription(subscriptionId) {
    ...
    if (subscription.flutterwaveSubscriptionId) {
      await flw.Subscription.cancel({ id: subscription.flutterwaveSubscriptionId });
    }
    subscription.status = 'inactive';
    subscription.gracePeriodEnd = new Date(Date.now() + 5 * 24 * 60 * 60 * 1000);
    await subscription.save();
    return subscription;
  }
  ...
  async renewSubscription(subscriptionId) {
    const subscription = await Subscription.findById(subscriptionId);
    if (subscription.status === 'inactive' && new Date() <= subscription.gracePeriodEnd) {
      subscription.status = 'pending';
      await subscription.save();
      return this.createSubscriptionPayment(subscription.userId, subscription.planId, subscription.paymentMethod);
    }
